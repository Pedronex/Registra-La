 ...
 */
export function Header({ title = 'Registra Lá', showConfig = true, back }: HeaderProps) {
  const logoDark = require('@/assets/Relogio.png')
  const { theme } = useTheme()
  const { updateAvailable } = useUpdate()

  const LogoContainer = () => (
    <View className={`p-2 w-14 h-14 rounded-2xl ${theme === 'dark' ? '' : 'bg-primary'}`}>
      <Image source={logoDark} className="w-full h-full" accessibilityLabel="Logo do aplicativo" />
    </View>
  )
  ...
  if (back) {
    return (
      <View className="flex-row justify-between items-center px-6 w-screen">
        {back()}
        <Text className="text-4xl font-medium text-surface-content" accessibilityRole="header">
          {title}
        </Text>
        <LogoContainer />
      </View>
