...
export function History() {
  ...
  /**
   * Renderiza a lista de registros de ponto
   */
  const renderRecords = () => (
    <View className="flex-col">
      {records.map((record) => (
        <View key={record.id} className="flex-row justify-between items-center py-2 border-b border-gray-300">
          <Text className="text-lg">{record.time}</Text>
          <Text className="text-lg">{record.description}</Text>
        </View>
      ))}
    </View>
  );
  ...
  return (
    <View className="p-4 w-full bg-white rounded-lg shadow-md">
      {renderHeader()}
      {loading ? <Text>Carregando...</Text> : renderRecords()}
      <View className="mt-4">
