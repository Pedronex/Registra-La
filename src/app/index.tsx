...
export default function InitialPage() {
  ...
  const renderSlogan = () => (
    <View>
      <Text className="text-3xl text-background-content text-start" accessibilityRole="text">
  ...
  const renderEnterButton = () => (
    ...
    >
      <Text className="text-3xl font-bold text-center text-primary-content">Entrar</Text>
    </Link>
  )

  return (
    <SafeAreaView className={`justify-between items-center p-10 w-screen h-screen bg-background`}>
      {renderHeader()}
      {renderSlogan()}
      {renderEnterButton()}
    </SafeAreaView>
