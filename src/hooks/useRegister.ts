...
export function useRegister() {
   ...
   */
  const extractDataPhoto = useCallback(
    async (base64: string, apiKey: string) => {
      setLoading(true);
  ...
  const editRegister = useCallback(
    async (id: number, data: Partial<RegisterInsert>) => {
      try {
        setLoading(true);
        setError(null);
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Error updating record";
        setError(errorMessage);
        throw err;
      } finally {
        setLoading(false);
  ...
  const findById = useCallback(async (id: number) => {
    try {
      ...
      const errorMessage =
        err instanceof Error ? err.message : "Error finding record";
      setError(errorMessage);
      throw err;
    } finally {
      setLoading(false);
